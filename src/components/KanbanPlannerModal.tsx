...
export function KanbanPlannerModal({
  ...
  const moveTask = (taskId: string, targetCol: ColumnId) => {
    ...
    (Object.keys(newCols) as ColumnId[]).forEach(col => {
      const idx = newCols[col].findIndex(item => item.id === taskId);
      if (idx !== -1) {
        itemToMove = newCols[col][idx];
        newCols[col] = newCols[col].filter(item => item.id !== taskId);
      }
    });

    if (itemToMove) {
      newCols[targetCol] = [itemToMove, ...newCols[targetCol]];
      setColumns(newCols);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4 animate-fadeIn">
      <div className="bg-slate-900 text-slate-100 rounded-2xl border border-slate-700/80 shadow-2xl w-full max-w-7xl h-[92vh] flex flex-col overflow-hidden">
              ...
              filterItems(columns.backlog).map(item => (
                <KanbanTaskCard
                  ...
                  actions={[
                    ...
                    {
                      label: 'Hủy 🚫',
                      color: 'bg-rose-600/80 hover:bg-rose-600 text-white',
                      onClick: () => moveTask(item.id, 'cancelled')
                    }
                  ]}
                />
              ))
        ...
        <div className="bg-slate-900 border-t border-slate-800 p-4 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={onClose}
              className="px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 text-xs font-semibold rounded-xl cursor-pointer transition-colors"
            >
              Hủy bỏ
            </button>
            ...
            <button
              ...
              disabled={columns.nextWeek.length === 0 && columns.completed.length === 0 && columns.cancelled.length === 0}
...
function KanbanTaskCard({ item, badgeText, badgeColor, onDragStart, actions }: KanbanTaskCardProps) {
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 pt-1.5 opacity-90 group-hover:opacity-100 transition-opacity">
        {actions.map((act, idx) => (
          <button
            key={idx}
            onClick={act.onClick}
