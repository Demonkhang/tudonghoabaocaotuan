const { createDocxReport } = require('../services/docxService');

// Mẫu dữ liệu chuẩn ban đầu
const sampleData = {
  metadata: {
    tuan: 42,
    tuan_tiep: 43,
...
exports.syncData = async (req, res) => {
  try {
    // Trả về dữ liệu chuẩn từ Google Sheet / Apps Script
    return res.json({
      success: true,
      data: sampleData,
...
exports.generatePdf = async (req, res) => {
  try {
    // Với PDF, trả về phản hồi JSON hỗ trợ chế độ xem in trực tiếp hoặc xuất PDF
    const reportData = req.body || sampleData;
    res.json({
      success: true,
